...
export type TVConfig<T extends Record<string, any>> = {
  ...
  [P in keyof T]?: {
    compoundVariants?: TVCompoundVariants<
      WidenVariantsValues<T[P]['variants']>,
      T[P]['slots'],
      ClassValue, object, undefined
    >
  };
}

type WidenVariantsValues<V extends Record<string, any> | undefined>
  = V extends Record<string, any> ? V & { [K in keyof V]: V[K] extends Record<string, any>
...
> = {
  [K in keyof T['variants']]: keyof T['variants'][K] extends 'true' | 'false' ? boolean : keyof T['variants'][K];
}
